...
import { goodModify, goodGetById } from '@/api/good'
...
export default {
  ...
  created() {
    categoryAll({}).then((response) => {
      ...
      return goodGetById({ id: this.$route.query.id })
    }).then((response) => {
      // 人工初始化temp
      this.temp = response.data
      for (const file of response.data.fileList) {
  ...
  methods: {
    ...
    modifyData() {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          goodModify(Object.assign({}, this.temp)).then(() => {
            this.$notify({
              title: '成功',
              message: '修改成功',
              type: 'success',
