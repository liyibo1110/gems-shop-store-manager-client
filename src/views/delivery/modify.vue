<template>
  <div class="app-container">
    <div class="filter-container" style="margin-bottom: 10px;">
      <el-form v-loading="loading" ref="dataForm" :model="temp" :rules="rules" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)" label-position="left" label-width="120px" style="width: 1100px; margin-left:50px;">
        ...
        <el-button type="primary" @click="modifyData()">修改</el-button>
      </el-form>
    </div>
...
import { deliveryModify, deliveryGetById } from '@/api/delivery'
...
export default {
  ...
  created() {
    allProvince({}).then((response) => {
      ...
      return deliveryGetById({ id: this.$route.query.id })
    }).then((response) => {
      this.temp = response.data
      if (this.temp.method === 10) {
        this.firstLabelName = '首件（个）'
        this.additionalLabelName = '续件（个）'
      } else if (this.temp.method === 20) {
  ...
  methods: {
    ...
    createRule() {
      if (this.tempRule.region.length === 0) {
    ...
    modifyRule() {
      if (this.modifyTreeCheckedKeys.length === 0) {
        this.$message('至少要选择1个城市')
        return
      }
    ...
    handleRuleEdit(index, row) {
      ...
      // this.modifyTempCities = Object.assign({}, row.region)
      ...
      // this.$refs.modifyTree.setCheckedKeys(row.region)
      // this.modifyTreeCheckedKeys = row.region
      console.log(this.modifyTreeCheckedKeys)
    },
    modifyData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          deliveryModify(Object.assign({}, this.temp)).then(() => {
            this.$notify({
              title: '成功',
              message: '修改成功',
              type: 'success',
